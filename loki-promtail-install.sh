...
configure_nginx() {
    ...
    if command -v htpasswd &>/dev/null; then
        sudo htpasswd -cb /etc/nginx/.htpasswd admin "admin" || error_exit "Failed to create password file for Nginx"
    else
...
EOL
    ...
    echo "Access Loki at http://<your-server-ip>:3009 with username 'admin' and password 'admin'"
}
...
configure_logrotate() {
    echo "Configuring logrotate for Loki and Promtail..."

    # Configure logrotate for Loki chunks and Promtail positions file
    cat <<EOL | sudo tee /etc/logrotate.d/loki
/var/lib/loki/chunks/* {
    daily
    rotate 10
    compress
    ...
    olddir /var/lib/loki/chunks/archived
}
